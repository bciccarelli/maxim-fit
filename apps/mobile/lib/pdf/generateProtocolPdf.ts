...
function buildMealHtml(meal: Meal): string {
  return `
    <div class="meal-item">
      ...
      <div class="meal-macros mono">
        P ${meal.protein_g}g · C ${meal.carbs_g}g · F ${meal.fat_g}g
      </div>
      ${
        meal.foods.length > 0
          ? `<div class="meal-foods">${meal.foods.map((f) => escapeHtml(f)).join(', ')}</div>`
          : ''
      }
    </div>
  `;
    ...
    <div class="card">
      ...
      <div class="macro-summary">
        <div class="macro-item">
          <span class="macro-value mono">${diet.daily_calories.toLocaleString()}</span>
          <span class="macro-label">KCAL</span>
        </div>
        <div class="macro-item">
          <span class="macro-value mono">${diet.protein_target_g}</span>
          <span class="macro-label">PROTEIN (G)</span>
        </div>
        <div class="macro-item">
          <span class="macro-value mono">${diet.carbs_target_g}</span>
          <span class="macro-label">CARBS (G)</span>
        </div>
        <div class="macro-item">
          <span class="macro-value mono">${diet.fat_target_g}</span>
          <span class="macro-label">FAT (G)</span>
        </div>
        <div class="macro-item">
          <span class="macro-value mono">${diet.hydration_oz}</span>
          <span class="macro-label">WATER (OZ)</span>
        </div>
      </div>
      <div class="meals-list">
        ${diet.meals.map(buildMealHtml).join('')}
      </div>
      ${
        ...
        <div class="notes-section">
          ...
          <ul class="notes-list">
            ${diet.dietary_notes.map((n) => `<li>${escapeHtml(n)}</li>`).join('')}
          </ul>
        </div>
      `
          : ''
    ...
    <div class="card">
      ...
      ${
        ...
        <div class="notes-section">
          <span class="notes-label">Notes</span>
          <ul class="notes-list">
            ${supplementation.general_notes.map((n) => `<li>${escapeHtml(n)}</li>`).join('')}
          </ul>
        </div>
      `
          : ''
  ...
  return `
    <div class="card">
      ...
      <div class="workouts-list">
        ${training.workouts.map(buildWorkoutHtml).join('')}
      </div>
      ${
        training.rest_days.length > 0
          ? `<div class="rest-days"><span class="rest-label">Rest days:</span> ${training.rest_days.map((d) => escapeHtml(d)).join(', ')}</div>`
          : ''
      }
      ...
      ${
        ...
        <div class="notes-section">
          <span class="notes-label">Notes</span>
          <ul class="notes-list">
            ${training.general_notes.map((n) => `<li>${escapeHtml(n)}</li>`).join('')}
          </ul>
        </div>
      `
          : ''
...
function buildProtocolHtml(protocol: DailyProtocol, metadata: ProtocolMetadata): string {
...
export async function generateProtocolPdf(
  ...
  const html = buildProtocolHtml(protocol, metadata);
